...
2. **L2 Instruction Verification** — First, before any manifest is consulted, confirms the request does not contradict itself: when `instruction_data` is supplied, the declared `instruction_discriminator` must be a prefix of it, and data shorter than the declared discriminator is a contradiction too. This runs for every program, manifested or not, and a failure is a hard gate. (Until Round 13 it ran only after a manifest HIT, so a discriminator the manifest did not know returned the unknown-instruction soft pass before the comparison was reached — which is how a real SetAuthority could be presented as an unknown instruction and clear the Risk Engine's known-risky table.) Then confirms the instruction discriminator and account count match the manifest's declared shape. **Every manifest lookup in the pipeline — this one, account resolution, the declared-effects lookup, the risk context and the plugin rules — is keyed on the instruction's own first eight bytes rather than on the caller's label** (Round 14). `discriminator_matches` is `input.starts_with(selector)`, so a label SHORTER than a selector misses it, and a short label contradicts nothing; keyed on the label, a caller could truncate its own discriminator and take `resolve_accounts` down the unknown-instruction path, whose synthesised accounts carry `pda_mismatch`, `expected_address_mismatch` and `privilege_mismatch` all false — switching off PDA re-derivation, the fixed-address comparison and the privilege comparison together. Keying on the bytes cannot refuse anything the label would not have: the contradiction check above has already established that the label is a prefix of the data, so every selector the label matched is also a prefix of these bytes. When `signed_transaction` is supplied, L2 additionally requires that the described instruction is *in* the bytes — an instruction under the described program carrying exactly the described data, whose accounts match the described accounts position by position, with lookup-table positions resolved (see "Transaction Artifact" below) — and that every other instruction in the message is declared in `transaction_instructions` (bijective sibling coverage; an undeclared sibling fails the layer). A durable-nonce transaction (instruction 0 is System `AdvanceNonceAccount`) fails L2 by default because it does not expire; see "Durable Nonces" below. L2 is a hard gate.
3. **L3 Simulation Verification** — Runs `simulateTransaction` and checks compute/account-write/CPI divergence. Active whenever an RPC client is attached (`GRAPHITE_RPC_URL`); the simulation-integrity module runs a 3-signal z-score (compute, writes, CPI hops) with Welford's algorithm against earned baselines, plus median/MAD baseline (C28) for poisoning resistance. Live-validated against real Solana devnet transactions (C40). Without an RPC client the layer reports an honest `Inconclusive` state, never a phantom pass. **Only the transaction artifact is ever simulated** (Round 19, F-19-02): a descriptive request — no `signed_transaction` — used to have its `instruction_data` sent to the simulator as if it were a transaction, and L3 certified the result. It is now reported as "no simulation". **What one observation is** (Round 19, F-19-01/03): a baseline sample is recorded only when the described instruction was located in the artifact, and it is keyed on `tx_artifact::simulation_identity` — the SHA-256 of the frame with its signature slots and its recent blockhash (a v1 frame's lifetime) zeroed, because `simulateTransaction` runs with `replaceRecentBlockhash` and the blockhash is not part of what executes. The same transfer under a fresh blockhash is therefore one observation, not a new one each time. Each program's accumulator remembers the identities it has counted exactly (64-bit fingerprints, bound 16,384 per program, persisted with the snapshot); a full memory stops the live baseline learning and routes new observations to the shadow accumulator — it never forgets, so an identity counted once is never counted again.
4. **L4 State Verification** — Diffs pre/post account state against the manifest's declared `expected_state_changes`. With an RPC client attached and a signed transaction to simulate, Graphite builds the diff itself: pre-state from `getMultipleAccounts`, post-state from `simulateTransaction`'s `accounts` request, over exactly the instruction's writable accounts. `state_diff.rs` decodes SPL Token and Token-2022 accounts and mints (using the Token-2022 account-type byte to tell an extended mint from an extended account) and reports what changed: lamport and token balance movement, ownership reassignment, account creation and closure, delegate and close-authority grants, freezes, mint supply changes, and — since Round 13 — the three SPL authority fields: the token account's `owner` (who may move the balance, which is a different field from the account's owning *program* that ownership reassignment watches), the mint authority and the freeze authority. A `SetAuthority(AccountOwner)` moves no value and leaves the owning program untouched, so before that the most direct account takeover on Solana produced a diff with no findings in it. Token-2022 extensions are **classified, not modelled**: the TLV region is scanned and each extension is `AltersTransferSemantics` (TransferFee, TransferHook, ConfidentialTransfer, …), `AltersAuthority` (PermanentDelegate, …), `Informational`, or `Unknown`. The first, second and fourth classes fail L4 (`Token2022ExtensionNotModelled`), an unreadable or truncated extension region fails it (`Token2022ExtensionRegionUnreadable` — an empty list is never inferred from unparseable data), and informational extensions warn. **The transfer fee is modelled** (Round 20): for every fee-bearing token account in the diff Graphite reads `TransferFeeAmount.withheld_amount` before and after, takes the mint's `TransferFeeConfig` — from the diff, or fetched by Graphite at pre-state, because a `TransferChecked` names its mint read-only and so never in the diff — and requires the observed movement to be exactly Token-2022's: every fee withheld at a destination is `TransferFee::calculate_fee` on one transfer of what arrived there under the older or the newer schedule; tokens arriving untaxed are ones the schedule does not charge; withheld fees leave an account only by being harvested into the mint, exactly; and the mint's value is conserved across the accounts covered. The fee and the arriving amount are stated (`Token2022TransferFeeCharged`), a fee larger than what arrives blocks (`Token2022TransferFeeMajority`), a higher pending schedule is disclosed with what would then arrive (`Token2022TransferFeeRising`), and an undeclared change to the fee terms blocks (`Token2022TransferFeeConfigChanged`). Anything the model cannot account for exactly — a withdrawal of withheld fees, several transfers into one account, an unreadable entry, a schedule it could not read — leaves the extension unmodelled on those accounts, and it blocks as before with the reason. **Which accounts L4 observes** (Round 20): every account the TRANSACTION makes writable, not only those the manifest expects to be — the fee payer signing as a transfer authority was otherwise left out of the diff, and every self-paid token transfer failed `ArtifactEffectsNotCovered`. **A diff from the request is the caller's** whatever `provenance` it claims (Round 20, F-20-01).
...
**Which tree is judged (Round 19, F-19-22).** Until Round 19 these rules ran only on a `cpi_trace` the caller chose to declare — and the reference bridge never declares one, so on real traffic they never ran. The Core now rebuilds the primary instruction's call tree from the simulation it runs itself: `innerInstructions` (program, accounts, data and `stackHeight` of every inner call) is parsed by `rpc_client`, nested by stack height (a missing height is read as a direct callee, a jump of more than one level is clamped to the next level so no executed call is dropped, and an index that cannot be resolved yields no tree rather than an empty one), and `tx_pattern_analysis::analyze_observed_cpi_trace` runs the same five rules on it. A Blocked finding refuses the transaction with the reason prefixed `observed CPI trace:`. Under a manifested root program an unknown callee is a warning rather than a block (the manifest is what vouches for the root); impersonation is still judged against the full known set. Token-2022 `TransferCheckedWithFee` and System `TransferWithSeed` count as transfers for the fan-out rule.
...
Reachable through `graphite quarantine add|lift|list` (operating on the server's durable graph, so a restart picks the change up) and `POST/GET /admin/quarantine` on a running server. The endpoint requires the operator key, `GRAPHITE_ADMIN_API_KEY` — a separate secret of at least 32 characters that must differ from the verify key — and is refused outright (`403`) when none is configured, including on a keyless `GRAPHITE_DEV_MODE=1` instance (Round 19, F-19-13: until then the verify key every agent holds could lift a quarantine). The program id must be a valid public key, the reason is capped at 1,024 characters, a repeat quarantine is a no-op (`already_quarantined`), and `persisted` reports whether the snapshot actually reached disk. The CLI opens the data directory under the same exclusive lock the server holds, so it refuses to write beside a running server rather than racing it; its read-only commands (`list`, `verify`, `protocol status`, …) open the directory without writing.
...
`[compact-u16 signature count][64-byte signatures][message]`, legacy and v0, and since
Round 19 the **v1** frame (SIMD-0385): `0x81`, the message, then
`num_required_signatures` × 64 signature bytes LAST with no count, up to 4,096 bytes, no
lookup tables, and the compute-budget settings (priority fee, compute-unit limit,
loaded-accounts data size, heap size) carried in the message as config values selected
by a u32 mask — with no `solana-sdk` dependency. Every runtime refusal of a v1 frame is
Graphite's too (unknown mask bits, more than 12 signatures / 64 addresses / 64
instructions, duplicate addresses, heap size not 1 KiB-aligned within 32–256 KiB, the
header arithmetic, trailing bytes), proven against agave's own wincode decoder and
sanitizer by `tools/runtime-oracle` with zero frames where Graphite is looser; every
frame helper (`unsigned_artifact`, `filled_signature_slots`, `message_bytes`,
`bound_artifact_sha256`, `simulation_identity`) finds a v1 frame's signatures at its end.
The static keys must be distinct (the bank's `AccountLoadedTwice`), a lookup may not
resolve an address the message already names or another lookup already loaded, and a
lookup account whose type tag is not an initialized table is refused (Round 19,
F-19-08). Every length is a canonical compact-u16 (three groups at most,
...
| **Request bounds** | 1 MiB body, 10 s request timeout that the RPC budget fits inside, identifier length caps on every caller-influenced field that reaches a log or the audit trail; `signed_transaction` ≤ 1232 bytes (`PACKET_DATA_SIZE`) for legacy/v0 and ≤ 4,096 for a v1 frame, ≤ 256 declared sibling instructions; the concurrency permit is taken only after rate limiting, authentication, and receipt of the whole body within 5 s, and a refusal drains for at most 1 s before closing (Round 19, F-19-14); `/audit/event` and `/verify/execution` require a 16-hex `content_hash`, bound the signature to 90 characters and `reported_by` / `audit_trail_id` / `detail` to 128 / 128 / 1024. |
| **CORS** | Denied by default; `GRAPHITE_CORS_ORIGINS` (comma-separated) enables specific browser origins. |
| **Audit trail** | Append-only JSONL under `GRAPHITE_DATA_DIR`. Every record is `sync_data`'d to the device before the response is sent; a verification whose record cannot be written is refused with `503` rather than answered. Rotation at `GRAPHITE_AUDIT_ROTATE_BYTES` (64 MiB) is a rename, never a rewrite; archives are kept unless `GRAPHITE_AUDIT_MAX_ARCHIVES` is set. Every reader — dashboard endpoints and L8's `last_verification_for` — covers the archives plus the active file, with per-archive statistics cached because archives are immutable; the L8 / lifecycle join is an in-memory index over the active file keyed three ways per verification row — `audit_trail_id`, `transaction_sha256` (the artifact digest, recorded on every artifact-bound row since Round 10) and `content_hash` — built at open, maintained per append, cleared on rotation, with archives scanned newest-first by the same key only on a miss. **L8 attributes an execution by the chain's bytes:** `getTransaction` (base64) → bound to the signature (`tx_artifact::bound_artifact_sha256`: the first slot holds it and it verifies, ed25519, over the message under the fee payer's key — bytes that fail are refused with `chain_bytes_rejected`, `Unavailable`, and no fallback; Round 11) → signature slots zeroed (`unsigned_artifact`, which reproduces the artifact the bridge sent byte for byte) → SHA-256 → `find_verification(TransactionSha256)`; the caller's `audit_trail_id` / `transaction_sha256` / `content_hash` stand in only when the chain's bytes cannot be fetched, most exact first and never falling back to a coarser one, and are cross-checked (`caller_keys_disagree`). `content_hash` alone names every transaction carrying that instruction, which is why it was the wrong join (Round 10). Lifecycle events reported by callers are stored as attestations with `reported_by`, every field bounded on the way to disk; each row carries `verdict_on_record` — what the trail said about the verification the event names, resolved the same most-exact-first way with contradicting keys refused — and `verdict_on_record_key`; a report against a blocked verdict is counted (`graphite_lifecycle_events_on_blocked_total`) and logged as loudly as an L8 discrepancy. No reader treats a lifecycle row as a verdict. **The trail knows its own order (Round 12):** lifecycle rows are indexed per transaction (`lc:id:` / `lc:tx:` / `lc:sig:` → offsets, at most 64 per key — the first 32 and the latest 32, so the newest row is never the one dropped (Round 19, F-19-20) — cleared on rotation with the newest archive consulted on a miss; a record written after a torn final line starts on a new line, and a line that is not UTF-8 skips itself rather than every record after it (F-19-23)), and `/audit/event` computes `sequence_anomalies` for every report against the rows on record — `duplicate` (a retry; the bridge retries a submission report up to three times), `out of order`, `unpreceded`, and `signature conflict` (a second signature attached to one transaction, which means one report is false; logged as loudly as a discrepancy). Rows Graphite writes from its own observation (the L8 row) are marked `observed_by_graphite` and count for ordering, never as duplicates of a caller's report; an L8 row attributed by `content_hash` carries no exact keys, because that attribution names *a* transaction carrying the instruction. From submission onward a report needs a `transaction_signature`, and a signature has to decode to 64 bytes. One server per data directory is enforced by an exclusive lock on `graphite.lock`. |
| **Durability** | Semantic-graph snapshot (trust tiers + earned simulation baselines + the observation memory) written atomically (temp file synced, then renamed) and reloaded on restart. Snapshot failures are counted. Since Round 19 (F-19-12) every snapshot carries a generation and an older one never replaces a newer one on disk; a snapshot that cannot be read refuses startup instead of starting fresh (which silently lifted every quarantine); and the data directory has exactly one writer — the CLI takes the same exclusive lock as the server for every command that writes, and opens read-only (no lock, no temp-file cleanup) for those that only read. |
...
- Does NOT prove inclusion cryptographically — two RPCs that agree are two views, not a light-client proof; the bytes behind a signature are proven the signature's, its presence in a block is what the endpoints say
...
- **The lifecycle is on the trail, in order, under the exact keys.** `executeBoundTransaction`
  ...
  /audit/event` `submission` → confirm → `POST /verify/execution` (L8). Every call carries
  `audit_trail_id` and `transaction_sha256` alongside `content_hash`. No irreversible
  step proceeds unless the step before it is on the trail: a signing that cannot be
  recorded, whose `verdict_on_record` is not `approved`, or that the server resolved by
  any key other than `audit_trail_id`, aborts before submission; after submission every
  failure is reported on `ExecutionOutcome.lifecycle` and none is hidden. Until Round 9
  the bridge recorded nothing after the verdict; until Round 10 the trail joined on the
  instruction-level key; until Round 11 the chain's bytes were whatever the RPC said they
  were — they are now accepted only when bound to the signature (first slot equal to it,
  verifying under the fee payer's key), and bytes that fail are refused with nothing
  attributed and no fallback to the caller's keys.
- **Swaps require the built payload.** Without the exact instruction (program id,
  ...
  aborts. The unverified-swap opt-out
  (`GRAPHITE_SWAP_ALLOW_UNVERIFIED_EXECUTION`) was retired in Round 19: it executed through
  SolanaAgentKit's own builder with the key SAK held, and SAK no longer holds one.
- **SolanaAgentKit holds no signing key (Round 19, F-19-C2).** SAK is constructed with
  `VerificationGatedWallet`: the public key, and every signing method throws
  `UngatedSigningRefused`. Before this, any SAK tool or plugin could sign and send outside
  the gate.
- **Nothing is signed before the verdict (Round 19, F-19-C1).** The bridge's own RPC
  simulation compiles an unsigned `VersionedTransaction` from the fee payer's public key
  and simulates it with `sigVerify: false` and `replaceRecentBlockhash: true`, and refuses
  to send bytes whose signature slots are not all zero. web3.js 1.x's
  `simulateTransaction(tx, signers)` used to sign the transfer on a live blockhash and hand
  the RPC a broadcastable transaction before Graphite had answered.
- **The intent is grounded in the user's words (Round 19, F-19-C5).** The AI layer's parse
  of a transfer is checked against amount and destination re-derived deterministically
  from the text, and must echo the text it was given; the intent the Core compares against
  is the grounded one.
...
│   ├── tests/              # 1,652 tests (unit + adversarial + exploit + RPC trust boundary + real mainnet legacy/v0/v1 + cross-language corpus)
│   └── Cargo.toml
├── sdk/
