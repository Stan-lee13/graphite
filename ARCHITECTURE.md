...
4. **L4 State Verification** — Checks the manifest's declared `expected_state_changes` for *structural consistency* with the resolved account list: fund-movement wording ("debits"/"credits") must be matched by at least two writable accounts, authority wording by a signer, and so on. **This is a consistency check, not a state diff.** It does not read on-chain balances and does not compare pre- and post-transaction account state — no balance snapshot exists anywhere in the pipeline. A manifest that declares changes inconsistent with the accounts it was given genuinely fails here (and that failure is a hard gate); a manifest that lies *consistently* is not caught by this layer. Real pre/post balance diffing requires per-account RPC snapshots around simulation and is tracked as a follow-up — see `SECURITY.md` → Known Limitations. (Corrected 2026-09-05: this line previously read "Diffs pre/post account state", which overstated what the code does. The honest description existed only in a source comment.)
