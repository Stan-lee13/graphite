...
use graphite_core::{
    solana_types::Pubkey,
    manifest::{load_seed_manifests, ManifestRegistry, ManifestError},
    verification::{GraphiteCore, VerificationInput, VerificationResult, ProposedIntent},
    risk_engine::{RiskAssessmentInput, RiskVerdict, RiskPattern, assess},
    confidence_engine::{compute_confidence, ConfidenceResult, TrustTier, WeightedSignal, SignalKind},
...
fn test_all_10_manifests_load_with_valid_pubkeys() {
    ...
    for m in &manifests {
        ...
        assert!(pubkey.is_ok(), 
            ...
            m.protocol.program_id, m.protocol.name);
    }
}
...
fn test_no_duplicate_program_ids() {
    let registry = load_seed_manifests();
    let manifests = registry.list();
    let mut ids: Vec<&str> = manifests.iter().map(|m| m.protocol.program_id.as_str()).collect();
    ids.sort();
    for i in 1..ids.len() {
...
fn test_every_instruction_has_expected_state_changes() {
    ...
    for m in registry.list() {
        for ix in &m.instructions {
            assert!(!ix.expected_state_changes.is_empty(),
...
fn test_memo_program_loaded_with_empty_discriminator() {
    ...
    assert!(memo.instructions[0].discriminator.is_empty(),
...
fn test_exactly_6_accounts_flagged_as_drainer() {
    let input = risk_input("program", &["a1","a2","a3","a4","a5","a6"], &[], &[], &[], "");
...
fn test_risk_engine_deterministic_1000_runs() {
    ...
    for _ in 0..1000 {
        assert_eq!(assess(&input).unwrap(), first, "risk engine is not deterministic");
    }
}
...
fn test_system_assign_blocked() {
    let input = risk_input("11111111111111111111111111111111",
        &["account","new_owner"], &[], &["assigns"], &[], "01000000");
    assert!(matches!(assess(&input).unwrap(), RiskVerdict::Blocked { pattern: RiskPattern::AuthorityHijack, .. }));
}
...
fn test_compositional_drain_5_distinct_not_flagged() {
    let input = risk_input("agg", &[], &["a","b","c","d","e"], &[], &["a","b","c","d","e"], "");
    assert_eq!(assess(&input).unwrap(), RiskVerdict::Passed);
}
...
fn test_verify_unknown_program_blocked() {
    ...
    let input = make_input("4Nd1mYbz1NQ8Tk6eX5N6w5eM6eX5N6w5eM6eX5N6w5eM", "02000000",
        &["7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU"], &[], WalletProfile::Conservative, no_evidence());
    let result = core.verify(&input).unwrap();
    assert!(!result.approved, "unknown program should not be approved");
    assert!(result.confidence <= 0.55, "unknown program confidence should be <= 0.55, got {}", result.confidence);
    assert!(result.unknown_protocol, "should be flagged as unknown protocol");
}

#[test]
fn test_verify_safe_system_transfer_approved() {
    let core = GraphiteCore::default();
    let input = make_input("11111111111111111111111111111111", "02000000",
    ...
    assert!(result.confidence > 0.5, "safe transfer should have confidence > 0.5, got {}", result.confidence);
}
...
fn test_verify_jupiter_swap_with_unlisted_cpi_blocked() {
    ...
    let result = core.verify(&input).unwrap();
    assert!(!result.approved, "Jupiter swap with unlisted CPI should be blocked");
    assert_eq!(result.risk_verdict.status, "Blocked");
}
...
fn test_verify_squads_multisig_create() {
    let core = GraphiteCore::default();
    let input = make_input("SQDS4ep65T869zMMBKyuUq6aD6EgTu8psMjkvj52pCf", "8faecbbfaecf93c5",
        ...
        &["11111111111111111111111111111111"], WalletProfile::Standard, good_evidence());
    let result = core.verify(&input).unwrap();
    assert_eq!(result.risk_verdict.status, "Clear",
        "Squads create with allowed System CPI should pass, got: {:?}", result.risk_verdict);
}
...
fn test_verify_stake_delegate_passes_risk() {
    let core = GraphiteCore::default();
    let input = make_input("Stake11111111111111111111111111111111111111", "02000000",
        &["7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU", "8qbHbw2BbbTHBW1sbeqakYXVKRQM8Ne7pLK7m6CVfeR",
          "DEb5yphxEaPc5BN118svVN4R3GFu9jKs31Gcv5yekjZx", "9WzDXwBbmkg8ZTbNMqJx8W5DkxUkq5PjAB8qjGp3q5J",
          "4Nd1mYbz1NQ8Tk6eX5N6g5eM6eX5N6g5eM6eX5N6g5eM", "11111111111111111111111111111111"],
        &[], WalletProfile::Standard, good_evidence());
    let result = core.verify(&input).unwrap();
    assert_eq!(result.risk_verdict.status, "Clear");
}
...
fn test_verify_token_2022_set_authority_blocked() {
    ...
    assert!(!result.approved, "Token-2022 SetAuthority should be blocked");
}
...
fn test_conservative_profile_requires_high_confidence() {
    ...
    assert!(matches!(result, PolicyVerdict::RejectedBelowThreshold { .. }), 
        "Conservative should reject at 0.6 confidence, got {:?}", result);
}
...
fn test_standard_profile_approves_moderate_confidence() {
    ...
    assert!(matches!(result, PolicyVerdict::Approved), 
        "Standard should approve at 0.75 confidence, got {:?}", result);
}
...
fn test_all_profiles_block_when_risk_detected() {
    for profile in &[WalletProfile::Conservative, WalletProfile::Standard, WalletProfile::Permissive] {
        ...
        assert!(matches!(result, PolicyVerdict::RejectedRiskEngineBlock), 
            "{:?} profile should block when risk detected, got {:?}", profile, result);
    }
}
...
fn test_all_10_program_ids_round_trip() {
    ...
    for m in registry.list() {
        ...
        let encoded = pk.to_base58();
        assert_eq!(encoded, m.protocol.program_id,
            "program_id round-trip failed for {}", m.protocol.name);
    }
}
...
fn test_all_protocols_verifiable() {
    ...
    for m in registry.list() {
        ...
        assert!(result.is_ok(), 
            ...
            m.protocol.name, m.protocol.program_id, ix.discriminator, result.err());
        let result = result.unwrap();
        assert!(result.manifest_found, 
            "manifest not found for {} ({})", m.protocol.name, m.protocol.program_id);
    }
}
...
fn test_protocol_category_diversity() {
    ...
    let registry = load_seed_manifests();
    let names: Vec<&str> = registry.list().iter().map(|m| m.protocol.name.as_str()).collect();
    
    // Native
    assert!(names.iter().any(|n| n.contains("System")), "missing System Program");
    assert!(names.iter().any(|n| n.contains("Stake")), "missing Stake Program");
    
    // Token
    assert!(names.iter().any(|n| n.contains("SPL Token")), "missing SPL Token");
    assert!(names.iter().any(|n| n.contains("Token-2022")), "missing Token-2022");
    
    // DEX
    assert!(names.iter().any(|n| n.contains("Raydium")), "missing Raydium");
    assert!(names.iter().any(|n| n.contains("Jupiter")), "missing Jupiter");
    assert!(names.iter().any(|n| n.contains("Orca")), "missing Orca");
    assert!(names.iter().any(|n| n.contains("Meteora")), "missing Meteora");
