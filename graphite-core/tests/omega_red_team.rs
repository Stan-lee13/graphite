...
use graphite_core::risk_engine::{assess, RiskAssessmentInput, RiskVerdict, RiskPattern};
use graphite_core::confidence_engine::{compute_confidence, WeightedSignal, SignalKind, TrustTier};
use graphite_core::policy_engine::WalletProfile;
use graphite_core::simulation_integrity::{check_simulation_integrity, ComputeUsage, SimulationIntegrityInput, ComputeBaseline};
...
fn exploit_l3_compositional_drain_bypass_4_targets() {
    ...
    let input = RiskAssessmentInput {
        ...
        cpi_targets: vec![drainer.into(), drainer.into(), drainer.into(), drainer.into()],
        expected_state_changes: vec![],
...
fn exploit_l6_nan_baseline_bypasses_simulation_check() {
    let input = SimulationIntegrityInput {
        ...
        simulation_usage: ComputeUsage { compute_units: 999999, account_writes: 100, cpi_hops: 50 },
        baseline: ComputeBaseline { mean_compute_units: f64::NAN, std_compute_units: 100.0, sample_count: 100 },
        divergence_threshold: 2.0,
    };
    let result = check_simulation_integrity(&input);
    assert!(result.is_err(),
        "L6 FIXED: NaN baseline mean now rejected.");
}
...
fn exploit_l6b_infinity_std_bypasses_simulation_check() {
    let input = SimulationIntegrityInput {
        program_id: "test".to_string(),
        simulation_usage: ComputeUsage { compute_units: 999999, account_writes: 100, cpi_hops: 50 },
        baseline: ComputeBaseline { mean_compute_units: 100.0, std_compute_units: f64::INFINITY, sample_count: 100 },
        divergence_threshold: 2.0,
    };
    let result = check_simulation_integrity(&input);
    assert!(result.is_err(),
        "L6b FIXED: Infinity std now rejected.");
}
...
fn exploit_l11_nan_confidence_bypass() {
    let signals = vec![
        WeightedSignal { kind: SignalKind::ManifestMatch, value: f64::NAN, weight: 0.3 },
        WeightedSignal { kind: SignalKind::SimulationMatch, value: 1.0, weight: 0.3 },
        WeightedSignal { kind: SignalKind::HistoricalVolume, value: 1.0, weight: 0.25 },
        WeightedSignal { kind: SignalKind::CommunityVerification, value: 1.0, weight: 0.15 },
    ];
    let result = compute_confidence(&signals, TrustTier::BattleTested);
    match result {
        Ok(_) => panic!("L11 FIXED CHECK FAILED: NaN signal should be rejected but produced confidence"),
...
fn exploit_l15_infinity_signal_rejected() {
    let signals = vec![
        WeightedSignal { kind: SignalKind::ManifestMatch, value: f64::INFINITY, weight: 0.3 },
        WeightedSignal { kind: SignalKind::SimulationMatch, value: 1.0, weight: 0.3 },
        WeightedSignal { kind: SignalKind::HistoricalVolume, value: 1.0, weight: 0.25 },
        WeightedSignal { kind: SignalKind::CommunityVerification, value: 1.0, weight: 0.15 },
    ];
...
fn exploit_l22_low_sample_count_skips_simulation_check() {
    ...
    assert!(result.simulation_flagged.is_none() || !result.simulation_flagged.unwrap(),
