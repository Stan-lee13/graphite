...
fn regression_l1_drainer_5_accounts_now_blocked() {
    let input = RiskAssessmentInput {
        ...
        accounts: vec![
            "victim_wallet".into(), "att1".into(), "att2".into(), "att3".into(), "att4".into(),
        ],
        cpi_targets: vec![],
...
fn regression_l4_token2022_setauthority_now_blocked() {
    ...
    assert!(matches!(result, RiskVerdict::Blocked { pattern: RiskPattern::AuthorityHijack, .. }),
        "L4 FIXED: SetAuthority on real Token-2022 ({}) now blocked", real_token2022);
}
...
fn regression_l4b_spl_token_setauthority_now_blocked() {
    ...
    let result = assess(&input).unwrap();
    assert!(matches!(result, RiskVerdict::Blocked { pattern: RiskPattern::AuthorityHijack, .. }),
        "L4b FIXED: SetAuthority on real SPL Token ({}) now blocked", real_spl_token);
}
