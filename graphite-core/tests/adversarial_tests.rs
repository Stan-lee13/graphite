...
use graphite_core::{
    ...
    solana_types::Pubkey,
    verification::{GraphiteCore, ProposedIntent, VerificationInput},
};
...
fn test_evasion_set_authority_on_token_2022_with_safe_intent() {
    ...
    assert!(
        !result.approved,
        "Token-2022 SetAuthority with safe intent must be blocked"
    );
    assert_eq!(result.risk_verdict.status, "Blocked");
}
...
fn test_evasion_drainer_with_safe_intent_and_few_accounts() {
    ...
    assert!(
        !result.approved,
        "CloseAccount must be blocked even with few accounts — it's in RISKY_PATTERNS"
    );
}
...
fn test_evasion_unknown_program_with_permissive_profile() {
    ...
    assert!(
        ...
        "Must be flagged as unknown protocol"
    );
    assert!(
        result.confidence <= 0.55,
        "Unknown protocol confidence must be <= 0.55, got {}",
        result.confidence
    );
}
...
fn test_evasion_unknown_program_with_high_evidence() {
    ...
    assert!(
        result.confidence <= 0.55,
        "Unknown protocol confidence cap must hold even with MAX evidence, got {}",
        result.confidence
    );
}
...
fn test_spoofing_system_transfer_with_authority_hijack_discriminator() {
    ...
    assert!(
        !result.approved,
        "System Assign must be blocked — it's authority hijack"
    );
    assert_eq!(result.risk_verdict.status, "Blocked");
}
...
fn test_spoofing_spl_token_transfer_discriminator_on_wrong_program() {
    ...
    assert!(
        !result.approved,
        "Unknown program must not be approved regardless of discriminator"
    );
...
fn test_boundary_confidence_exactly_0_55_unknown_protocol() {
    ...
    let result = core.verify(&input).unwrap();
    assert!(
        result.confidence <= 0.55 + 0.001, // floating point tolerance
        "Unknown protocol confidence must be <= 0.55, got {}",
        result.confidence
    );
}
...
fn test_chaining_drainer_plus_authority_hijack_plus_cpi() {
    ...
    assert!(
        ...
        "Multiple risk patterns must result in block"
    );
}
...
fn test_chaining_drainer_plus_hidden_transfer() {
    ...
    assert!(
        matches!(result, RiskVerdict::Blocked { .. }),
        "Drainer + hidden transfer must block"
    );
}
...
fn test_replay_1000x_deterministic() {
    ...
    for _ in 0..1000 {
        assert_eq!(
            assess(&input).unwrap(),
            first,
            "Risk engine must be deterministic across 1000 runs"
        );
    }
}
...
fn test_replay_verification_1000x_deterministic() {
    ...
    for i in 0..1000 {
        ...
        assert_eq!(
            ...
            "approved changed on iteration {}",
            i
        );
        assert_eq!(
            result.confidence, first.confidence,
            "confidence changed on iteration {}",
            i
        );
...
fn test_confusion_almost_system_program() {
    ...
    assert!(
        !result.approved,
        "Almost-System-Program must not be approved"
    );
    assert!(
        result.unknown_protocol,
        "Must be flagged as unknown — it's NOT the real System Program"
    );
}
...
fn test_confusion_almost_spl_token() {
    ...
    assert!(
        result.unknown_protocol,
        "Must be unknown — wrong program ID"
    );
}
...
fn test_confusion_capital_vs_lowercase_program_id() {
    ...
    assert!(
        !result.approved,
        "Case-modified program ID must not be approved"
    );
    assert!(
        result.unknown_protocol,
        "Must be unknown — case-sensitive mismatch"
    );
}
...
fn test_policy_no_profile_overrides_risk_block() {
    ...
    for profile in &[
        ...
        assert!(
            ...
            "{:?} must not override risk block even with confidence 1.0",
            profile
        );
    }
}
...
fn test_manifest_injection_extra_instruction_not_loaded() {
    ...
    assert!(
        result.is_ok(),
        "Loading should succeed (it's a valid manifest format)"
    );
...
fn test_adversarial_squads_with_non_system_cpi_blocked() {
    ...
    assert!(
        !result.approved,
        "Squads with non-System CPI must be blocked"
    );
}
...
fn test_full_pipeline_safe_transfer_approved_with_audit_trail() {
    ...
    assert!(
        !result.audit_trail_id.is_empty(),
        "Audit trail ID must be generated"
    );
...
fn test_full_pipeline_blocked_transaction_has_audit_trail() {
    ...
    assert!(
        !result.audit_trail_id.is_empty(),
        "Blocked transactions must still have audit trail ID"
    );
}
...
fn test_full_pipeline_unknown_protocol_has_audit_trail() {
    ...
    assert!(
        !result.audit_trail_id.is_empty(),
        "Unknown protocol must still have audit trail ID"
    );
    assert!(result.unknown_protocol);
}
