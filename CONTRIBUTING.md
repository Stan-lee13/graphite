...
- [ ] `cargo test --release` passes (0 failures — currently 1,652 tests, 15 ignored; `--include-ignored` for the live RPC corpus and the mainnet sample)
- [ ] `cargo test --release --no-default-features --lib` (327) and `--no-default-features --features cli` (1,436) pass — the feature matrix is tested, not just checked
