...
Updated: 2026-09-21, after Round 16 (see `git log -1 -- docs/CURRENT.md`).
...
| **Round 16 open findings (audit only, no code changed):** an artifact whose signature slots already hold real signatures is accepted and approved as `artifact_bound` with `transaction_sha256 = SHA256(raw)`; L8 joins by `SHA256(zeroed slots)` and consults only that key when the chain's bytes are available, so a REFUSED pre-signed artifact that executes reconciles as `NoVerificationOnRecord`, `discrepancy: false` — the same query with the RPC unable to return bytes says `BlockedButExecuted` (F-16-01, Medium; F-15-04 with its consequence measured); ten identical-CU simulations — approved or not, even L2-refused or `err != null` — freeze a program's baseline at zero variance and every honest transaction at any other CU is then Blocked `SimulationSpoofing` until an operator reseeds (F-16-02, Medium; F-16-03 is the eligibility gap that makes it cheap); CPI targets are declared by the caller although `innerInstructions` carries the real ones (F-16-05); PDA templates fall back instead of refusing (F-16-06); a panicking third-party Risk plugin loses its block (F-16-07); `min_confidence > 1` is a 500 (F-16-08); `content_hash` is unframed (F-16-09); the console renders manifest URLs as links without a scheme check (F-16-10); L8 digest attribution is last-wins (F-16-11); CI `cargo test` lacks `--locked` (F-16-13). See the Round 16 report | **Open** | Engineering |
