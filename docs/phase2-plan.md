...
- **844 Rust tests**, 0 failures; clippy 0 warnings; fmt clean; no-default-features builds
...
- **L3/L8 honest states**: L3 provenance-aware tri-state (`Passed`/`Failed`/`Inconclusive`); L8 honestly "live-validated against mainnet RPC (C40)" with audit-trail event
...
| Self-Healing | Yes — operator-triggered quarantine | — | The Phase 1 anomaly-detection module was removed as dead code; the semantic graph's `quarantine()` primitive survived but nothing called it. Wired 2026-09-05: `GraphiteCore::quarantine_program`/`lift_program_quarantine`, `graphite quarantine add\|lift\|list`, `POST/GET /admin/quarantine`, and a `ProgramQuarantined` hard gate in `verify`. Fixed three defects found while wiring it: `trust_tier.max(manifest_tier)` silently discarded the downgrade, an ordinary `append` cleared an active quarantine (so publishing a new manifest version was a self-service restore), and a program with no prior record could not be quarantined at all. Deliberately NOT automatic — see ARCHITECTURE.md for the P14 tradeoff |
...
- **Verification:** 987 tests / 0 failures,
...
- **Verification:** 6 endpoint integration tests (data correctness, read-only,
  auth), 844 total / 0 failures, clippy clean, fmt clean, all CI feature
...
- [x] All 844 existing tests pass (no regressions)
