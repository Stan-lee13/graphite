...
### C19. P16 REAL-MAINNET BENCHMARK + REAL EXPLOIT CORPUS — TWO REAL DEFECTS FOUND & FIXED
- **Deliverable:** the first P16-compliant precision/recall run on unseen real data. `mainnet-benchmark.ts` now fetches live mainnet txs from known protocols (Jupiter, Squads, Raydium CLMM) with the intent a real agent would attach, and replays a NEW real pinned exploit corpus (`tests/fixtures/exploit_corpus.json`) — 35 transactions pinned by signature from documented phishing accounts (SolPhishHunter arXiv:2505.04094; STMT/AAT/ISA attack classes). Full report: `docs/p16-mainnet-benchmark.md`.
- **Result:** malicious recall **100%** (35/35 real exploits blocked, 0 missed); legitimate approvals 0/7 on a fresh node — every block root-caused: 3× Jupiter conf 0.44 cold-start ceiling, 3× Squads conf 0.04 cold-start + tier, 1× Raydium CLMM unknown-protocol (no manifest yet).
- **C19.1 (real bug, fixed): the 64-account input cap rejected legitimate modern transactions.** `verify_async` capped account lists at 64 as a DoS guard; a real 72-account Jupiter V6 route tx was rejected with `Account count mismatch: expected 64, got 72`. Cap raised to Solana's protocol limit (256). Regression test proves the exact 72-account route verifies and approves once the node carries earned evidence.
- **C19.2 (real gap, fixed): ISA (system-account impersonation) was NOT detected.** The corpus proved a System transfer to a vanity `…11111` / `Compu…` address was blocked only incidentally (low confidence), risk=Clear. Added **P0 Check 10** (`Impersonation` pattern): fund-movement instructions (System transfer 0x02, Token transfer 0x03/transferChecked 0x0c) to/from an address impersonating an official system account are blocked. Heuristic grounded in the paper's criteria (vanity 11111 suffix / Compu prefix; ~1-in-2^40 for random keys). Corpus test asserts the blocks are principled.
- **C19.3 (fixed): the old harness carried fabricated addresses.** `KNOWN_DRAINER_ADDRESSES` had two invalid IDs (`MarBmsSgKXdrN1egZf5sqeX2qBfXviuenNATHCx5p8V1`, `dRiftyHA9M48UKS5r9rBZhxHbgnZ9uVnMwYvPwR9pR1e`) rejected by the RPC itself (WrongSize). Replaced with the real pinned corpus; real Marinade/Drift IDs verified on-chain.
- **Honest cold-start finding (documented, by design):** on a fresh node the confidence engine's earned-evidence signals are zero (P7), so the max achievable confidence for a manifest-matched tx is 0.44 — below every production profile threshold. Steady-state approval is proven by the seeded regression test; a live deployment accrues evidence over time and attaches an RPC client for L3 simulation.
