...
### C25 — Orca full-surface rebuild: 6 fabricated instructions removed (2026-08-09)

Round four (independent audit of C24) found that C24's Orca "correction" was itself built on a fabricated premise: 6 of the 24 manifest entries (`updateFeeRate`, `transferPositionDelegate`, `applyDelta`, `syncTickArray`, `closeAccount`, `closeConfigExtension`) never existed in any deployed version of the program — they appear in neither the 2022-era deployed IDL (v0.1.0, 25 instructions) nor the current deployed IDL (66 instructions), and the orca-so/whirlpools git history has zero occurrences. The manifest is rebuilt from the official deployed-program IDL (npm `@orca-so/whirlpools`, program v0.9.0) with all 66 discriminators as explicit byte arrays, not re-derived hashes.

| # | Finding | Class | Fix |
|---|---|---|---|
| C25.1 | **6 of 24 Orca manifest entries were fabricated** — `updateFeeRate`, `transferPositionDelegate`, `applyDelta`, `syncTickArray`, `closeAccount`, `closeConfigExtension` match no deployed program version (v0.1.0 = 25 instructions, current = 66) and have zero occurrences in the full orca-so/whirlpools git history. The C24 note claimed these were "corrected to the snake_case convention" — a claim resting on the false premise that they existed | Root (discriminator fabrication, second order: fabricated entries survived C24's camelCase fix) | All 6 removed; regression test asserts each name is ABSENT from the manifest and that the stale C24-era values do not resolve |
| C25.2 | **Manifest covered only 24 of 66 deployed instructions** — every legitimate Orca txn using any other instruction (two_hop_swap, collect_protocol_fees, bundled positions, set/fee/close config ops, …) fell to unknown-protocol mode (0.55 ceiling) | Root (incomplete surface) | Rebuilt from the official deployed-program IDL (program v0.9.0): all 66 discriminators are the IDL's explicit byte arrays; IDL committed as `scripts/whirlpool_idl.json`; manifest version 2.0.0; `scripts/rebuild_orca_manifest.py` regenerates |
| C25.3 | **On-chain corroboration limited to 2 instructions in C24** | Evidence gap | Extended live census (base58-correct, 528 txs): `swap = f8c69e91e17587c8` (×153), `swap_v2 = 2b04ed0b1ac91e62` (×342), `increase_liquidity_by_token_amounts_v2 = effb097cd2c6352b` (×7 — exists only in the current IDL, proving the IDL is the deployment's instruction set); census script `scripts/census_orca.py` |
